...
Input readInput(int argc, char **argv)
{
    if (argc < 4)
    {
        std::cerr << "Not enough arguments!  Use: 18-1 <width> <height> <num-coords-to-use>" << std::endl;
        std::exit(1);
    }
    ...
    input.take = std::atoi(argv[3]);
    if (!input.width || !input.height || !input.take)
    {
        std::cerr << "One or more arguments invalid!  Use: 18-1 <width> <height> <num-coords-to-use>" << std::endl;
        std::exit(1);
    }
